...
from imitation_learning.utils import D4RLTrajectoryDataset, evaluate_on_env, get_d4rl_normalized_score
from imitation_learning.model import BCAgent
...
def train(args):
    ...
    for i_train_iter in range(max_train_iters):
        ...
        for _ in range(num_updates_per_iter):
            try:
                timesteps, states, actions, traj_mask = next(data_iter)
            ...
            ################ Gaussian Noise #####################
            state_noise = torch.randn_like(states) * 0.2
            states += state_noise.clamp(-0.5,0.5)

            action_noise = torch.randn_like(actions) * 0.2
            actions += action_noise.clamp(-0.5,0.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', type=str, default='sdt_runs/')
