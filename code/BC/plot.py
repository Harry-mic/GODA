...
def plot(args):
    ...
    ax = plt.gca()
    ax.set_title('SafetyGym(8*Hazards)')
    ...
    if plot_avg:
        ...
        for filename in all_files:
            frame = pd.read_csv(filename, index_col=None, header=0)
            print(filename, frame.shape)
            frame['y_smooth'] = frame[y_key].rolling(window=y_smoothing_win).mean() *0.9
        ...
        #ax.axhline(y=30, ls="--", c="black", linewidth=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_d4rl_name', type=str, default='merge')
