...
#先训好一个z*model：
    for i_iter in range(max_train_iters):
        ...
        print(log_str)
    s6s10states = feature_class[:,0:170].reshape(64,10,17)*state_std+state_mean#s6s10
    s6_s10_states = recon_mu[:,0:170].reshape(64,10,17)*state_std+state_mean#s6'~s10'

    # s6__s10__states = recon_mu2[:,0:85].reshape(64,5,17)
    # s6_s10_actions = recon_mu[:,85:115].reshape(64,5,6)
    # new_rewards = recon_mu[:,115:120].reshape(64,5,1)
    s6s10states = pd.DataFrame(s6s10states[63,:,:].detach().cpu().numpy())
    s6_s10_states = pd.DataFrame(s6_s10_states[63,:,:].detach().cpu().numpy())
    # s6__s10__states = pd.DataFrame(s6__s10__states[63,:,:].detach().cpu().numpy())
    s6s10states.to_csv("~/code/GODA/exp/exp5/exp5code/s6~s10.csv")
    s6_s10_states.to_csv("~/code/GODA/exp/exp5/exp5code/s6'~s10'.csv")
    print(state_mean)
    print(state_std)
    # s6__s10__states.to_csv("~/code/GODA/yuce/s6''~s10''.csv")
...
#******************************************************************************************数据增强的部分************************************************************************
    ...
    for i_train_iter in range(max_train_iters):
        ...
        for _ in range(num_updates_per_iter):
            ...
            #将采样出的数据输入增强网络********************************************************************************************************************************
            states = states.reshape(star_batch_size ,-1).to(device)          # B x T x state_dim     [64,340]                                                       # 
            actions = actions.reshape(star_batch_size ,-1).to(device)        # B x T x act_dim       [64,120]                                                        #
            rewards = rewards.reshape(star_batch_size ,-1).to(device)        #                       [64,20]                                                        #
        ...
        #替换数据                                                                                                                                                    #
            ...
            states = states.reshape(64,20,17).to(device)
